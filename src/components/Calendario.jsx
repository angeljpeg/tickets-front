import { useState } from "react";
import {
  format,
  startOfMonth,
  endOfMonth,
  startOfWeek,
  endOfWeek,
  addDays,
  addMonths,
  subMonths,
  isSameDay,
  isSameMonth,
  parseISO,
} from "date-fns";

const Calendar = () => {
  const [currentMonth, setCurrentMonth] = useState(new Date());
  const [selectedDate, setSelectedDate] = useState(null); // Para mostrar en el modal
  const [appointments, setAppointments] = useState([
    {
      id: 0,
      fechaInicio: "2024-10-18T10:00:00",
      fechaFin: "2024-10-18T11:00:00",
      idTicket: 9,
    },
    {
      id: 1,
      fechaInicio: "2024-11-18T10:00:00",
      fechaFin: "2024-11-18T11:00:00",
      idTicket: 10,
    },
    {
      id: 2,
      fechaInicio: "2024-11-20T14:00:00",
      fechaFin: "2024-11-20T15:30:00",
      idTicket: 12,
    },
    {
      id: 3,
      fechaInicio: "2024-11-25T09:00:00",
      fechaFin: "2024-11-25T10:30:00",
      idTicket: 15,
    },
    {
      id: 4,
      fechaInicio: "2024-11-25T12:30:00",
      fechaFin: "2024-11-25T10:30:00",
      idTicket: 16,
    },
  ]);

  const startDate = startOfWeek(startOfMonth(currentMonth));
  const endDate = endOfWeek(endOfMonth(currentMonth));

  const days = [];
  let day = startDate;

  while (day <= endDate) {
    days.push(day);
    day = addDays(day, 1);
  }

  const handlePreviousMonth = () => {
    setCurrentMonth(subMonths(currentMonth, 1));
  };

  const handleNextMonth = () => {
    setCurrentMonth(addMonths(currentMonth, 1));
  };

  const formatAppointments = (date) =>
    appointments.filter((appt) => isSameDay(parseISO(appt.fechaInicio), date));

  return (
    <div className="p-8">
      <div className="flex items-center justify-between mb-4">
        <button
          onClick={handlePreviousMonth}
          className="p-2 transition-all duration-300 rounded bg-neutral-600 hover:bg-golden hover:text-black"
        >
          Anterior
        </button>
        <h2 className="text-2xl font-bold">
          {format(currentMonth, "MMMM yyyy")}
        </h2>
        <button
          onClick={handleNextMonth}
          className="p-2 transition-all duration-300 rounded bg-neutral-600 hover:bg-golden hover:text-black"
        >
          Siguiente
        </button>
      </div>

      <div className="grid grid-cols-7 gap-2 text-center">
        {["Dom", "Lun", "Mar", "Mié", "Jue", "Vie", "Sáb"].map((day) => (
          <div key={day} className="font-bold text-neutral-500">
            {day}
          </div>
        ))}

        {days.map((date, index) => {
          const appointmentsOnDate = formatAppointments(date);

          return (
            <div
              key={index}
              className={`p-4 rounded-lg transition-all duration-300 hover:bg-opacity-70 hover:cursor-pointer ${
                isSameMonth(date, currentMonth)
                  ? "bg-neutral-700"
                  : "bg-neutral-800"
              } ${
                isSameDay(date, new Date()) ? "bg-neutral-400 text-white" : ""
              }`}
              onClick={() => setSelectedDate(appointmentsOnDate)}
            >
              <div className="text-sm font-bold">{format(date, "d")}</div>
              {appointmentsOnDate.map((appt) => (
                <div
                  key={appt.id}
                  className="px-1 mt-1 text-xs rounded bg-neutral-200 text-neutral-700"
                >
                  {format(parseISO(appt.fechaInicio), "HH:mm")}
                </div>
              ))}
            </div>
          );
        })}
      </div>

      <div className={`${selectedDate ? "opacity-100" : "opacity-0"} duration-300`}>
        {selectedDate && (
          <div className="fixed inset-0 flex items-center justify-center bg-black bg-opacity-75">
            <div className="p-4 rounded-lg shadow-2xl bg-neutral-800">
              <h3 className="text-lg font-bold">Detalles de las citas</h3>
              {selectedDate.length > 0 ? (
                <ul>
                  {selectedDate.map((appt) => (
                    <li key={appt.id} className="mt-2">
                      <p>
                        <strong>Hora:</strong>{" "}
                        {format(parseISO(appt.fechaInicio), "HH:mm")} -{" "}
                        {format(parseISO(appt.fechaFin), "HH:mm")}
                      </p>
                      <p>
                        <strong>ID Ticket:</strong> {appt.idTicket}
                      </p>
                    </li>
                  ))}
                </ul>
              ) : (
                <p>No hay citas para este día.</p>
              )}
              <button
                onClick={() => setSelectedDate(null)}
                className="p-2 mt-2 transition-all duration-300 rounded bg-neutral-600 hover:bg-golden hover:text-black"
              >
                Cerrar
              </button>
            </div>
          </div>
        )}
      </div>
    </div>
  );
};

export default Calendar;
